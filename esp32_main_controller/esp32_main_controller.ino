...
#define VOICE_GAS_ALERT 6       // "Gas bahaya" (Opsional jika mau dobel sama buzzer)
...
void setup() {
  ...
  if (dfPlayer.begin(dfSerial)) {
    ...
    playVoice(VOICE_WELCOME); // Test bunyi
  } else {
    dfPlayerReady = false;
    Serial.println("[WARN] DFPlayer Not Found (Check wiring)");
  }
...
void loop() {
  ...
  // --- LOGIC BUZZER (Hanya untuk GAS - Dari Backend) ---
  if (buzzerState) {
...
void controlDoor(bool lock, String method) {
  ...
  // --- LOGIKA SUARA PINTU (Remote/App) ---
  if (lock) {
    playVoice(VOICE_DOOR_LOCKED); // "Pintu Terkunci"
    doorAutoLockPending = false;
  } else {
    playVoice(VOICE_DOOR_UNLOCKED); // "Pintu Terbuka"
    // Auto lock timer start
  ...
  if (key) {
    if (key == '#') {
      if (enteredPin == CORRECT_PIN) {
        ...
        delay(1500); // Tunggu suara selesai

        // 2. Buka Pintu (Ini akan trigger suara "Pintu Terbuka")
        controlDoor(false, "pin");
        
        // 3. Tambahan Suara Welcome
        delay(1500);
        ...
        playVoice(VOICE_WRONG_PIN); // Suara Salah
...
void checkDoorAutoLock() {
  if (doorAutoLockPending && !doorLocked) {
    if (millis() - doorUnlockTimer >= DOOR_UNLOCK_DURATION) {
      controlDoor(true, "auto"); // Akan bunyi "Pintu Terkunci"
